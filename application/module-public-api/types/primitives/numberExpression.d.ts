 ...
 *
 * Comparison methods produce lazy {@link ConditionExpression} nodes that
 * integrate with boolean logic (and / or / negate / ifTrue / ifFalse).
 *
 ...
 *
 * From the JavaScript side, a NumberExpression wrapper is always truthy.
 * Implicit numeric coercion should throw — the underlying value is hidden;
 * only the declared operations may be executed against it.
 *
 * @see specification/expressions/numberExpression.md
 * @see NumberExpressionApi
 */
export type NumberExpression = {
  /**
   * Convenience factory: create a literal NumberExpression node.
   *
   * Delegates to {@link NumberExpressionApi.of}.
   */
   ...
   * Arithmetic addition.
   *
   * Lazy: evaluates the receiver, then `other`. Computes the sum with
   * unbounded precision, reduces modulo 2^64, and interprets the result
   * as a signed two's-complement long. Overflow wraps (Java-style).
   *
   ...
   *
   * Lazy: evaluates receiver minus other with unbounded precision,
   * reduces modulo 2^64, interprets as signed long. May be implemented
   * as addition of the negated right-hand side.
   */
   ...
   *
   * Lazy: multiplies with unbounded precision, reduces modulo 2^64.
   * Large products wrap according to modulo semantics.
   */
   ...
   *
   * Lazy: evaluates both operands and performs integer division.
   * Recommended rounding: truncate toward zero (3 / 2 = 1, -3 / 2 = -1).
   *
   ...
   *
   * Selects uniformly from [fromInclusive, toInclusive] using the
   * runtime's randomness context (ExecutionContext). Results are
   * deterministic and reproducible for a given context.
   *
 ...
 * values.
 *
 * Exposed as `hostApi.number` (or `number` destructured from hostApi) inside
 * module scripts.
 *
...
export type NumberExpressionApi = {
   ...
   * - `NaN` -> throws at call time
   * - `+/-Infinity` -> throws at call time
   * - Non-integers -> truncated toward zero before conversion
   * - After truncation, value is reduced into the 64-bit two's-complement range
   *
   * The result is cached. NumberExpression nodes are opaque wrappers in JS;
   * callers must not rely on implicit numeric coercion.

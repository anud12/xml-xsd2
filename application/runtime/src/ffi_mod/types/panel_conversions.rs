...
fn empty_panel(id: String, pj: *mut c_char) -> PanelFfi {
    PanelFfi {
        id: cstr(id), background: cstr(String::new()),
...
/// Convert a Vec<String> of panel JSON into C PanelFfi array.
pub unsafe fn panels_to_c_array(
    panels: Vec<String>,
) -> (*mut PanelFfi, usize) {
    if panels.is_empty() {
        ...
        let ffi = if p.trim_start().starts_with('{') {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&p) {
                PanelFfi {
                    ...
                    anchor: AnchorFfi {
                        x: g(&v["anchor"]["x"]), y: g(&v["anchor"]["y"]),
                    },
                    pivot: AnchorFfi {
                        x: g(&v["pivot"]["x"]), y: g(&v["pivot"]["y"]),
                    },
                    offset: OffsetFfi {
                        top: g(&v["offset"]["top"]),
                        bottom: g(&v["offset"]["bottom"]),
                        left: g(&v["offset"]["left"]),
                        right: g(&v["offset"]["right"]),
                    },
                    size: SizeFfi {
                        height: g(&v["size"]["height"]),
                        width: g(&v["size"]["width"]),
                    },
                    children_json: std::ptr::null_mut(),
                    panel_json: pj,
                }

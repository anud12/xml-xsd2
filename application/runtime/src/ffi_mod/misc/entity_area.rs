...
/// The ids of the entities inside the named entity's area, serialized as a
/// JSON array of strings (id ascending, self excluded, deduped across
/// containers). An entity with no area — or that is not a member of any
/// container — yields `"[]"`. The caller frees the returned string with
/// [`runtime_free_string`].
#[no_mangle]
pub extern "C" fn runtime_get_entities_inside_area(
    ...
    let ids = crate::state::entities_inside_area(&id_str, &containers);
...
/// Register (or clear) an entity's area polygon from its serialized vertices.
/// The vertices arrive as a JSON array of `[x, y]` pairs in entity-local
/// units. An empty array clears the area. Idempotent and order-independent:
/// the geometry is recomputed from the raw polygon.
#[no_mangle]
pub extern "C" fn runtime_set_entity_area(
    id: *const c_char,
    polygon_json: *const c_char,
) {
    if id.is_null() || polygon_json.is_null() {
        return;
    }
    ...
    crate::state::set_entity_area(&id_str, poly);
}
...
mod tests {
    ...
    fn ffi_set_area_then_query_roundtrips() {
        ...
        // room covers (10,10)..(110,110); the local rect is 0..100.
        let room_poly = c("[[0,0],[100,0],[100,100],[0,100]]");
        unsafe { runtime_set_entity_area(c("room").as_ptr(), room_poly.as_ptr()) };
        crate::state::set_last_containers(vec![container(
            "c1",
            &[("room", 10.0, 10.0), ("npc-in", 30.0, 40.0), ("npc-out", 500.0, 500.0)],
        )]);

        let out = unsafe { runtime_get_entities_inside_area(c("room").as_ptr()) };
        assert!(!out.is_null());

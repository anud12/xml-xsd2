...
pub extern "C" fn runtime_run_iteration(elapsed_units: i64) -> i64 {
    ...
    helpers::log("runtime_run_iteration, CALLING process_behavior_machine");
    if let Err(e) = crate::js_executor::behavior_scripts::process_behavior_machine(total) {
        helpers::log(&format!("Failed to process behavior machine: {:?}", e));
    }
    helpers::log("runtime_run_iteration, RETURNED from process_behavior_machine");

    helpers::log("runtime_run_iteration, CALLING process_active_plans");
    crate::js_executor::process_active_plans(total);
    helpers::log("runtime_run_iteration, RETURNED from process_active_plans");

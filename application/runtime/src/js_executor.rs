...
fn effect_context_js() -> &'static str {
  ...
  function makeValueWrapper(v, entityRef, keyRef) {
    var wrapper = {
      map: function(cb) { return cb(wrapper); },
      sum: function(addend) {
        var nv = (v || 0) + addend;
...
pub fn process_pending_effects(files: &std::collections::HashMap<String, String>) -> Result<()> {
    ...
    // Log to file for debugging since C callback may not fire in Maven context
    use std::io::Write;
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("C:\\temp\\rust_debug.log") {
        let _ = writeln!(f, "[{}] process_pending_effects: effects_count={}, effects={:?}", std::process::id(), effects.len(), effects);
    }
    ...
    let _ = ctx.with(|c| c.eval::<(),_>(r#"globalThis.string={of:function(s){return s;}};globalThis.number={of:function(n){return n;}}"#));
    ...
    // Build entity store from __entityData so effect execution can find entities.
    let _ = ctx.with(|ctx| ctx.eval::<(), _>(r#"globalThis.__entityStore=[];if(globalThis.__entityData){for(var id in globalThis.__entityData){var e=JSON.parse(JSON.stringify(globalThis.__entityData[id]));e.textMap_name=id;globalThis.__entityStore.push(e);}}"#));
    ...
    for effect_name in effects.iter() {
        ...
        let mut script = String::new();
        script.push_str(effect_context_js());
        script.push_str("(function(){var evs=globalThis.__registeredEffects||[];var f=null;for(var i=0;i<evs.length;i++){if(evs[i]&&typeof evs[i]==='object'&&evs[i].name==='");
        script.push_str(effect_name);
        script.push_str("'){f=evs[i];break;}}globalThis.__logs=globalThis.__logs||[];if(f){globalThis.__logs.push('DEBUG_PPE: found effect, hasApply='+(typeof f.apply==='function'));if(typeof f.prepare==='function')try{f.prepare({})}catch(ex){globalThis.__logs.push('DEBUG_PPE: prepare error:'+ex.message)}if(typeof f.apply==='function'){try{var ectx=buildEffectContext();globalThis.__logs.push('DEBUG_PPE: calling apply');f.apply(ectx);globalThis.__logs.push('DEBUG_PPE: apply succeeded');globalThis.__logs.push('DEBUG_PPE: entityStore after apply='+JSON.stringify(globalThis.__entityStore));}catch(e){globalThis.__logs.push('DEBUG_PPE: apply error:'+e.message+e.stack)}}}else{globalThis.__logs.push('DEBUG_PPE: effect not found');}})();");
        match ctx.with(|c| c.eval::<(),_>(script.clone())) {
            Ok(()) => {},
            Err(e) => {
                if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("C:\\temp\\rust_debug.log") {
                    let _ = writeln!(f, "[{}] EFFECT_EVAL_ERROR: {}", std::process::id(), e);
                    let _ = writeln!(f, "[{}] EFFECT_SCRIPT (first 500 chars): {}", std::process::id(), &script[..500.min(script.len())]);
                }
            }
        }
    }
    ...
    // Log entity data state for debugging
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("C:\\temp\\rust_debug.log") {
        let _ = writeln!(f, "[{}] entity_data_after_effects: {}", std::process::id(), &entity_data_json[..200.min(entity_data_json.len())]);
        let _ = writeln!(f, "[{}] effect_execution_logs: {}", std::process::id(), &logs_json[..500.min(logs_json.len())]);
    }

    if let Ok(entities_val) = serde_json::from_str::<serde_json::Value>(&entity_data_json) {
        // Extract the "after" field which contains the updated entity data
        let after_data = entities_val.get("after").unwrap_or(&entities_val);

        if let Some(entities_obj) = after_data.as_object() {
            ...
            if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("C:\\temp\\rust_debug.log") {
                let _ = writeln!(f, "[{}] merged entity number data: {:?}", std::process::id(), &number_data);
            }
...
fn convert_store_values(values: &[serde_json::Value]) -> Vec<Vec<String>> {
    ...
    for obj in values.iter(){
        if let Some(map)=obj.as_object(){
            if !map.is_empty(){
                // Prefer textMap_name as the entity identifier; fall back to first value

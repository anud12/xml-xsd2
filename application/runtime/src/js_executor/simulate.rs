...
pub fn simulate_action(
    ...
    persist_planned_moves(action_name, &sim);
    Ok((sim.created, convert_store_values(&sim.store)))
}

/// Park any move/teleport the action recorded (via `ctx.moveTo`/`teleportTo`)
/// as a `crate::state` active plan so the per-tick walker can advance it. The
/// actor is the moved entity id; a teleport is an immediate position write.
fn persist_planned_moves(action_name: &str, sim: &Sr) {
    if sim.planned_moves.is_empty() {
        return;
    }
    let total = crate::state::get_elapsed_time_units();
    for m in sim.planned_moves.iter() {
        let actor = m.entity_id.clone();

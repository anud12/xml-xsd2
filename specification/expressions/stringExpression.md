...
**StringOperations** is the factory and operation builder:
```typescript
export type StringApi = {
  /** Create a literal string (returns StringExpression immediately) */
  of: (s: string) => StringExpression,
  
  /** Build an operation: concatenate */
  concat: (other: StringExpression) => StringOperations,
  
  /** Build an operation: join with separator */
  join: (parts: StringExpression[], separator?: StringExpression) => StringOperations,
  
  /** Build an operation: deterministic choice among alternatives */
  oneOf: (choices: StringExpression[]) => StringOperations,
  
  /** Build an operation: reference another rule by id */
  ref: (ruleId: string) => StringOperations,
  
  /** Evaluate this operation sequence against a given value */
  evaluate: (value: StringExpression) => StringExpression,
  
  /** Register and retrieve named string rules */
  asRule: (ruleName: string, expr: StringExpression) => StringOperations,
  getRule: (ruleName: string) => StringOperations,
...
**StringExpression** is the lazy expression tree (composition only):
```typescript
export type StringExpression = {
  /** Apply an operation to transform the current value. Returns self for chaining. */
  apply: (operation: StringOperations) => StringExpression,
  
  /** Replace the current value entirely (reset point). Returns self for chaining. */
  set: (value: StringExpression) => StringExpression,
...
- **`StringExpression` is immutable** with an operation queue. The underlying string value never changes; only the queued operations grow.
